...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="Hồ sơ tài khoản" subtitle="Thông tin cá nhân, vai trò và trạng thái chủ xe của bạn trên nền tảng." />
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <article className="rounded-2xl border border-white/10 bg-slate-900/60 p-5">
          ...
          <div className="mt-4 grid gap-3 text-sm text-slate-200 md:grid-cols-2">
            ...
            <p className="flex items-center gap-2">Vai trò: <RoleBadge role={role} /></p>
        ...
        <article className="rounded-2xl border border-white/10 bg-slate-900/60 p-5">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-200">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-950/40 p-3">
              ...
              <span>Admin thẩm định hồ sơ theo quy trình 2 lớp.</span>
            </div>
            <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-950/40 p-3">
              <ShieldCheck className="h-4 w-4 text-emerald-300" />
              <span>Sau khi duyệt, bạn mở được Cổng chủ xe và đăng phương tiện.</span>
            </div>
          </div>

          <div className="mt-4 flex flex-wrap gap-2">
            {ownerStatus === 'APPROVED' ? (
              <Link to="/owner/dashboard" className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950">Mở Cổng chủ xe</Link>
            ) : ownerStatus === 'PENDING' ? (
              <Link to="/app/owner-application-status" className="rounded-xl border border-amber-400/30 bg-amber-500/10 px-4 py-2 text-sm font-semibold text-amber-200">Xem hồ sơ đang duyệt</Link>
            ) : (
              <Link to="/app/become-owner" className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950">Đăng ký làm chủ xe</Link>
            )}
          </div>
        </article>
      </div>
