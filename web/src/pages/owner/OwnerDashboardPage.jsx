...
export default function OwnerDashboardPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setLoading(true);
      try {
        const [vehicleRes, rentalRes, paymentRes, disputeRes] =
        ...
        setVehicles(
          vehicleRes.status === "fulfilled"
            ? pickArray(vehicleRes.value.data)
            : [],
        );
        setRentals(
          rentalRes.status === "fulfilled"
            ? pickArray(rentalRes.value.data)
            : [],
        );
        setPayments(
          paymentRes.status === "fulfilled"
            ? pickArray(paymentRes.value.data)
            : [],
        );
        setDisputes(
          disputeRes.status === "fulfilled"
            ? pickArray(disputeRes.value.data)
            : [],
        );
      } finally {
        setLoading(false);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          subtitle={`${activeVehicles} xe khả dụng`}
          icon={CarFront}
        />
        <StatCard
          title="Yêu cầu thuê"
          value={loading ? "..." : rentals.length}
          subtitle={`${pendingRequests} đang chờ`}
          icon={ClipboardList}
        />
        <StatCard
          title="Doanh thu"
        ...
        <StatCard
          ...
          subtitle="Yêu cầu bồi thường đang xử lý"
          icon={Scale}
        />
      </div>
            ...
            {rentals.slice(0, 5).map((rental) => (
              ...
              >
                ...
                <p className="text-xs text-slate-400">
                  Xe {String(rental.vehicle_id).slice(-6)}
                </p>
              </div>
            ))}
            {!rentals.length ? (
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Link
          to="/owner/vehicles/new"
          className="rounded-xl border border-cyan-400/30 bg-cyan-500/10 px-4 py-3 text-sm text-cyan-100 hover:bg-cyan-500/20"
        >
          Đăng xe mới
        </Link>
        <Link
          to="/owner/requests"
          className="rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-slate-200 hover:bg-white/10"
        >
          Xem yêu cầu từ người thuê
        </Link>
        <Link
          to="/owner/payments"
          className="rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-slate-200 hover:bg-white/10"
        >
          Xem thanh toán nhận được
        </Link>
        <Link
          to="/app/explore"
          className="rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-slate-200 hover:bg-white/10"
