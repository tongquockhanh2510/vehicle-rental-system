  ...
  useEffect(() => {
    const loadVehicle = async () => {
      ...
      try {
        ...
        setForm({
          vehicle_type:
            normalizeVehicleTypeValue(vehicle.vehicle_type) || "CAR",
          brand: vehicle.brand || "",
          model: vehicle.model || "",
          year: vehicle.year || "",
          ...
          allowed_region:
            vehicle.allowed_region || getAllowedRegionByCity(vehicle.city),
          description: vehicle.description || "",
          available_from: vehicle.available_from
            ? String(vehicle.available_from).slice(0, 10)
            : "",
          available_to: vehicle.available_to
            ? String(vehicle.available_to).slice(0, 10)
            : "",
        });
        ...
        pushToast({
          ...
          title: "Tải thất bại",
          message:
            error?.response?.data?.error || "Không thể tải thông tin xe.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editing) {
        ...
        if (files.length) {
          pushToast({
            ...
            message:
              "Thông tin xe đã cập nhật. Việc tải thêm ảnh hiện được quản lý ở API riêng.",
          });
        } else {
          pushToast({
            tone: "success",
        ...
        if (!files.length) {
          pushToast({
            tone: "warning",
            title: "Thiếu ảnh",
            message: "Vui lòng thêm ít nhất một ảnh xe.",
          });
          setSubmitting(false);
          return;
        }
        ...
        Object.entries(normalizePayload()).forEach(([key, value]) =>
        ...
        pushToast({
          ...
          message: "Xe đã được đăng thành công.",
        });
      }
      ...
      pushToast({
        tone: "error",
        title: "Lưu thất bại",
        message: error?.response?.data?.error || "Không thể lưu thông tin xe.",
      });
    } finally {
      setSubmitting(false);
    }
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="text-sm text-slate-300">
            ...
            <input
              ...
              onChange={(event) =>
                setField("license_plate", event.target.value)
              }
              className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-white outline-none"
            />
          </label>
          ...
          <label className="text-sm text-slate-300">
            ...
            <input
              ...
              onChange={(event) =>
                setField("pickup_location", event.target.value)
              }
              className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-white outline-none"
          ...
          <label className="text-sm text-slate-300">
            ...
            <input
              ...
              onChange={(event) =>
                setField("return_location", event.target.value)
              }
              className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-white outline-none"
          ...
          <label className="text-sm text-slate-300">
            ...
            <select
              ...
              onChange={(event) =>
                setField("allowed_region", event.target.value)
              }
              className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-white outline-none"
            >
          ...
          <label className="text-sm text-slate-300">
            ...
            <input
              ...
              onChange={(event) =>
                setField("available_from", event.target.value)
              }
              className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-white outline-none"
            />
          </label>
          ...
          <label className="text-sm text-slate-300">
            ...
            <input
              ...
              onChange={(event) => setField("available_to", event.target.value)}
              className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-white outline-none"
            />
          </label>
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-2">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400 disabled:bg-slate-600"
          >
