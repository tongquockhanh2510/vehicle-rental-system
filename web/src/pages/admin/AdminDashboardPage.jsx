  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setError(
          serviceError?.message || "Không thể tải dữ liệu dashboard admin.",
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className="space-y-6">
      ...
      {fallback ? (
        <div className="rounded-2xl border border-amber-400/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          Một số API quản trị chưa sẵn sàng, dashboard đang hiển thị dữ liệu dự
          phòng để không gián đoạn demo vận hành.
        </div>
      ) : null}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard
          title="Người thuê"
          value={loading ? "..." : metrics.totalRenters.toLocaleString("vi-VN")}
          icon={UserCog}
        />
        <StatCard
          title="Chủ xe đã duyệt"
          value={
            loading ? "..." : metrics.approvedOwners.toLocaleString("vi-VN")
          }
          icon={UserCheck}
        />
        <StatCard
          title="Hồ sơ chủ xe chờ duyệt"
          value={
            loading
              ? "..."
              : metrics.pendingOwnerApplications.toLocaleString("vi-VN")
          }
          icon={FileText}
        />
        <StatCard
          ...
          value={
            loading ? "..." : metrics.totalVehicles.toLocaleString("vi-VN")
          }
          icon={CarFront}
        />
        <StatCard
          title="Yêu cầu thuê chờ duyệt"
          value={
            loading ? "..." : metrics.pendingRentals.toLocaleString("vi-VN")
          }
          icon={ClipboardList}
        />
        <StatCard
          ...
          value={
            loading ? "..." : metrics.activeContracts.toLocaleString("vi-VN")
          }
          icon={FileText}
        />
        <StatCard
          title="Tổng thanh toán"
          value={loading ? "..." : formatCurrency(metrics.totalPayments)}
          icon={DollarSign}
        />
        <StatCard
          title="Phí nền tảng 4%"
          value={loading ? "..." : formatCurrency(metrics.systemFeeRevenue)}
          icon={Wallet}
        />
        <StatCard
          title="Tranh chấp chờ xử lý"
          value={
            loading ? "..." : metrics.pendingDisputes.toLocaleString("vi-VN")
          }
          icon={ShieldAlert}
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-2xl border border-white/10 bg-slate-900/60 p-5">
            ...
            {revenueByMonth.length ? (
              revenueByMonth.map((item) => {
                ...
                const width = Math.max(
                  8,
                  Math.round((value / maxRevenue) * 100),
                );
                return (
          ...
          <div className="mt-3 space-y-2">
            {pendingOwnerApps.length ? (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-semibold text-white">
                      {item.applicant_name ||
                        item.owner_profile?.legal_name ||
                        "Chưa cập nhật"}
                    </p>
                    <StatusBadge
                      status={`OWNER_${String(item.status || "PENDING").toUpperCase()}`}
                    />
                  </div>
              ...
              <p className="text-sm text-slate-400">
                Không có hồ sơ chủ xe chờ duyệt.
              </p>
            )}
          </div>
        </article>
        ...
        <article className="space-y-5">
              ...
              {topVehicles.length ? (
                topVehicles.slice(0, 5).map((item, idx) => (
                  <div
                    ...
                    className="flex items-center gap-3 rounded-xl border border-white/10 bg-slate-950/40 p-2"
                  >
                    <img
                      src={resolveImage(
                    ...
                    <div className="flex-1 text-sm">
                      <p className="font-semibold text-white">
                        {item.brand || "Xe"} {item.model || ""}
                      </p>
                      <p className="text-xs text-slate-400">
                        Lượt thuê:{" "}
                        {item.rental_count || item.total_rentals || 0}
                      </p>
                    </div>
                  </div>
                ))
            ...
            <div className="mt-3 space-y-2">
              {pendingDisputes.length ? (
                pendingDisputes.slice(0, 4).map((item) => (
                  <div
                    key={item._id}
                    className="rounded-xl border border-white/10 bg-slate-950/40 p-2 text-xs text-slate-200"
                  >
                    ...
                    <p className="mt-1 text-slate-400 line-clamp-2">
                      {item.description || "Chưa cập nhật mô tả tranh chấp."}
                    </p>
                  </div>
                ))
              ) : (
                <p className="text-sm text-slate-400">
                  Không có tranh chấp chờ xử lý.
                </p>
              )}
            </div>
          </div>
            ...
            <div className="mt-3 space-y-2">
              {recentRentals.length ? (
                recentRentals.slice(0, 4).map((item) => (
                  <div
                    key={item._id}
                    className="flex items-center justify-between rounded-xl border border-white/10 bg-slate-950/40 p-2 text-xs text-slate-200"
                  >
          ...
          <div className="rounded-2xl border border-cyan-400/25 bg-cyan-500/10 p-3 text-xs text-cyan-100">
            Snapshot hệ thống: API Gateway, MongoDB, Redis, RabbitMQ và JWT được
            theo dõi ở trang System Health (route kỹ thuật).
          </div>
        </article>
      </div>

      {loading ? (
        <div className="text-sm text-slate-400">Đang tải dashboard...</div>
      ) : null}
